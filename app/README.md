# Front-end для сервиса сопровождения покупок в Европе

## Команды для использования в docker

### Запуск dev-сервера
В директории проекта:  
```shell
 docker run -ti -v $(pwd):/usr/src/app -p 8086:8080 node:16-alpine \
 sh -c 'cd /usr/src/app && yarn; yarn serve'
```

Сервер будет доступен по адресу [http://localhost:8086](http://localhost:8086)  

### Сборка проекта
В директории проекта:  
```shell
 docker run -ti -v $(pwd):/usr/src/app -p 8086:8080 node:16-alpine \
 sh -c 'cd /usr/src/app && yarn; yarn build'
```

Готовые файлы будут доступны в ```./distr```

### Конфигурация nginx для демонстрации
Предполагается nginx на внешнем серевере и сконфигурованые сертификаты по адресу [https://demo.host](https://p.1qr.org)  
```shell
cat > /etc/nginx/sites-available/demo.host.conf

server {
    server_name demo.host;
    root /var/www/demo.host;
    access_log  /var/www/demo.host/access.log;
    error_log   /var/www/demo.host/error.log;
    ...
    ssl_certificate /etc/letsencrypt/live/demo.host/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/demo.host/privkey.pem; # managed by Certbot
    ssl_trusted_certificate /etc/letsencrypt/live/demo.host/chain.pem; # managed by Certbot
}
```
